...
MAX_VARIANT_PRICE = float(os.environ.get("MAX_VARIANT_PRICE", "5.00"))
MIN_VARIANT_PRICE = float(os.environ.get("MIN_VARIANT_PRICE", "0.50"))
PRODUCT_PAGES = 4          # 4 * 250 = up to 1000 products scanned
SKIP_WORDS = ('gift', 'donation', 'donate', 'tip', 'shipping', 'warranty',
              'bundle', 'mystery', 'e-gift', 'insurance', 'protection',
              'subscription', 'membership', 'deposit', 'sample pack')
SKIP_TYPES = ('gift card', 'gift_card', 'service', 'donation')
...
async def _get_products_page(session, domain, page, proxy):
    """One /products.json page. Returns list, or a string error code."""
    paths = (f"{domain}/products.json?limit=250&page={page}",
             f"{domain}/collections/all/products.json?limit=250&page={page}")
    last = "UNREACHABLE"
    for path in paths:
        for attempt in range(3):
            try:
                async with session.get(path, proxy=proxy, timeout=aiohttp.ClientTimeout(total=15),
                                       headers=_browse_headers(domain)) as resp:
                    if resp.status == 429 or resp.status in (430, 503):
...
async def fetch_products(domain, proxy_str=None):
    """Scan the WHOLE store and return the true cheapest in-stock variant."""
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            for page in range(1, PRODUCT_PAGES + 1):
                res = await _get_products_page(session, domain, page, proxy)
        ...
        if not all_products:
            if first_err in ("RATE_LIMITED",):
                return False, "RATE_LIMITED"
            if first_err == "NOT_SHOPIFY":
                return False, "<b>Not Shopify!</b>"
            return False, f"<b>Site Error! {first_err or 'No Products!'}</b>"

        min_price, min_product = None, None
        fallback_price, fallback_product = None, None

        for product in all_products:
            title = str(product.get('title', '')).lower()
            ptype = str(product.get('product_type', '')).lower()
            junk = any(w in title for w in SKIP_WORDS) or ptype in SKIP_TYPES
            ...
            for variant in product.get('variants') or []:
                ...
                if junk:
                    if fallback_price is None or price < fallback_price:
                        fallback_price, fallback_product = price, entry
                    continue
                if min_price is None or price < min_price:
                    min_price, min_product = price, entry

        if min_product is None:
            min_price, min_product = fallback_price, fallback_product

        if not min_product:
            return False, "<b>No Valid Products</b>"
        if min_price > MAX_VARIANT_PRICE:
            return False, f"<b>Cheapest ${min_price:.2f} over ${MAX_VARIANT_PRICE:.2f} cap</b>"
        return min_product
